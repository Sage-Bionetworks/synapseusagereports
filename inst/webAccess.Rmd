...
qTemplate <- 'select ENTITY_ID,CONVERT(AR.TIMESTAMP, CHAR) AS TIMESTAMP,DATE,USER_ID,NODE_TYPE,N.NAME from ACCESS_RECORD AR, PROCESSED_ACCESS_RECORD PAR, NODE_SNAPSHOT N, PROJECT_STATS.%s NODE where AR.RESPONSE_STATUS=200 AND MONTH(FROM_UNIXTIME(AR.TIMESTAMP/1000))=%s AND YEAR(FROM_UNIXTIME(AR.TIMESTAMP/1000))=%s and AR.SESSION_ID = PAR.SESSION_ID and AR.TIMESTAMP = PAR.TIMESTAMP and PAR.ENTITY_ID = NODE.ID and N.ID = NODE.ID and CLIENT = "WEB" AND (PAR.NORMALIZED_METHOD_SIGNATURE IN ("GET /entity/#/bundle", "GET /entity/#/version/#/bundle", "GET /entity/#/wiki2", "GET /entity/#/wiki2/#"));'
...
dataaccessCount <- queryData %>% 
  ...
  dplyr::select(name, everything(), -id, -NAME)

dataaccessCount %>% knitr::kable()
```