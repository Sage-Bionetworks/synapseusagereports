...
#### New and unique users per month, starting from first month in the range:
```{r}
...
#### Unique users per team (per month, starting from first month in the range):
```{r}
queryData %>% 
...
There are `r length(setdiff(unique(queryData$userName), c("anonymous")))` active registered Synapse users in this time period. Of these, `r multiMonthVisits(queryData) %>% nrow` users have viewed a page in at least two different months.
