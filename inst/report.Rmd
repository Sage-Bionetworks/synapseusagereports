...
foo <- DBI::dbSendQuery(conn=con, statement='DROP TABLE PROJECT_STATS.1773109;')
...
queryData <- rbind(queryDataPageviews, queryDataDownloads)

```
### Activity on Synapse project `proj@properties$name` (`r params$projectId`) from `r min(timestampBreaksDf$date)` to `r max(timestampBreaksDf$date)`.
...
#### Active new and unique users per month, starting from first month in the range:
```{r}
...
#### Active users per team (per month):
```{r}
queryData %>% 
