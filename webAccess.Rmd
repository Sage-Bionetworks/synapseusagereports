...
doQuery <- function(con, template, projectId, beginTimestamp, endTimestamp) {
  ...
  data.browse %>% filter(RESPONSE_STATUS == 200)  %>% 
...
```{r domysqlquery}
res <- ddply(monthBreaksDf, .(beginTimestamp, endTimestamp),
             function (x) doQuery(template=qBrowseTemplate, 
                                  con=con,
                                  projectId=projectId, 
...
aclUserList$teamId <- factor(aclUserList$teamId, 
                             levels=c(paste0("syn", projectId), 3323597, 3330234, 3332397),
                             ordered=TRUE)
